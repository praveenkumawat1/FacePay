 ...
 * Body:
 *  - faceEmbedding: number[]
 *  - sessionToken: string (optional — mobile QR session ke liye) ✅ NEW
 *
 * Sabhi active FaceData se compare karke best match user return karta hai
 */
router.post("/login", async (req, res) => {
  try {
    const { faceEmbedding, sessionToken } = req.body; // ✅ sessionToken added
    ...
    // ✅ NEW: Agar sessionToken hai to PC ko WebSocket se notify karo
    if (sessionToken && global.pcClients) {
      const pcWs = global.pcClients.get(sessionToken);
      if (pcWs && pcWs.readyState === 1) {
        ...
        console.log(
          `✅ PC notified via WebSocket (faceRoutes) for session: ${sessionToken}`,
        );
      } else {
        console.log(`⚠️  No PC WebSocket found for token: ${sessionToken}`);
      }
    }
    // ✅ END NEW
